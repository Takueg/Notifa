  ...
  task generate_posts: :environment do
    ...
    puts "scraping for ebay-kleinanzeigen posts"
      ...
      post_doc.search(card_selector).each do |card|
        ...
        # category = show_doc.search(category_selector)
        ...
        Post.create(title: title, address: address, price: price, size: size, post_url: post_url, image_urls: image_urls, room: room, description: description, date_posted: date_posted, company: company)
      end
      # return (title: title, address: address, price: price, size: size, post_url: post_url, image_url: image_url, room: room, category: category, description: description, date_posted: date_posted, company: company)
  end
end
